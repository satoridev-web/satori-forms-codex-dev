...
$classes  = array( 'satori-field', 'satori-field--date', $field['wrapper_class'] ?? '' );
$attrs    = '';
...
<div class="<?php echo esc_attr( trim( implode( ' ', array_filter( $classes ) ) ) ); ?>">
    ...
    <?php endif; ?>
    <input type="date" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field['name'] ); ?>" class="satori-field__control <?php echo esc_attr( $field['css_class'] ?? '' ); ?>" value="<?php echo esc_attr( $value ); ?>" <?php echo ! empty( $field['required'] ) ? 'required' : ''; ?><?php echo $attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> />
